...
public class ModulesCommand implements ICommand {
    ...
    public List<String> tabComplete(@NotNull CommandSender sender, @NotNull String[] args) {
        ...
        if (args.length <= 0) {
            completions.addAll(Arrays.stream(Modules.values()).map(Enum::name).map((s) -> s.toLowerCase(Locale.ROOT)).collect(Collectors.toList()));
        } else if (args.length <= 2)
            StringUtil.copyPartialMatches(args[1], Arrays.stream(Modules.values()).map(Enum::name).map((s) -> s.toLowerCase(Locale.ROOT)).collect(Collectors.toList()), completions);
